<?php
/**
 * Some slight tweaks made by Simon East:
 *   - commented out the exit() call when class is finished
 *   - adjusted the imageconvolution function to reduce sharpness just a tad
 */ 
...
if(! defined('MEMORY_LIMIT') )				define ('MEMORY_LIMIT', '250M');							// Set PHP memory limit
...
if(! defined('CURL_TIMEOUT') )				define ('CURL_TIMEOUT', 120);							// Timeout duration for Curl. This only applies if you have Curl installed and aren't using PHP's default URL fetching mechanism.
if(! defined('WAIT_BETWEEN_FETCH_ERRORS') )	define ('WAIT_BETWEEN_FETCH_ERRORS', 30);				// Time to wait between errors fetching remote file
...
class timthumb {
	...
	protected function processImageAndWriteToCache($localImage){
		...
		//------------ Hack by Simon ----------------------
		// Don't allow UPSCALING, where new width or height is greater than the original
		// but only on "fit" mode.  When using crop-mode, we will still permit enlargements
		if ($zoom_crop === 3 && $new_width > $width) {
		    $new_width = $width;
		}
		if ($zoom_crop === 3 && $new_height > $height) {
		    $new_height = $height;
		}
		...
		if ($sharpen && function_exists ('imageconvolution')) {
			...
			$sharpenMatrix = array (
					array (0,-1,0),
					array (-1,18,-1),		// increase this to go softer
					array (0,-1,0),
					);

			$divisor = 14;		// sum of all numbers above
			$offset = 0;
